...
func (p *Particle) tick() {
	for i := range p.position {
		p.speed[i] += p.acceleration[i]
		p.position[i] += p.speed[i]
	}
}

func (p Particle) collide(other Particle) bool {
	for i := range p.position {
		if p.position[i] != other.position[i] {
			return false
		}
	}
	return true
...
func (p Particle) totalAcceleration() int {
	return abs(p.acceleration[0]) + abs(p.acceleration[1]) + abs(p.acceleration[2])
}

func (p Particle) distanceAfter(t int) int {
	var totalDistance int
	for i := range p.position {
		totalDistance += abs(p.position[i] + p.speed[i] * t + p.acceleration[i] * t * (t+1)/2)
	}
...
func parseNumbers(row string) ([3]int) {
	var res [3]int
	numbers := strings.Split(row[3:len(row)-1], ",")
...
func ParseParticle(row string) Particle {
	chunks := strings.Split(row, ", ")
	return Particle{
		parseNumbers(chunks[0]),
		parseNumbers(chunks[1]),
		parseNumbers(chunks[2]),
	}
}
...
func realParticles() []Particle {
	var particles []Particle
	for _, row := range realInput() {
		particles = append(particles, ParseParticle(row))
	}
	return particles
}
...
func part1() {
	minI := -1
	minDist := 0
	const time = 1000000 // Random point far away in time
	for i, p := range realParticles() {
		if p.distanceAfter(time) < minDist || minI < 0 {
			minI = i
			minDist = p.distanceAfter(time)
		}
	}
	fmt.Printf("Eventually closest to <0,0,0> is particle #%v distnce is %v\n", minI, minDist)
}

func part2() {
	particles := realParticles()
	for i:=0; i < 100000; i++ { // Magic number... Just waiting for everything to collide
		particlesMap := map[[3]int][]Particle{}
		for _, p := range particles {
			p.tick()
			particlesMap[p.position] = append(particlesMap[p.position], p)
		}
		var newParticles []Particle
		for _, particles := range particlesMap {
			if len(particles) == 1 {
				newParticles = append(newParticles, particles[0])
			} else {
				fmt.Printf("Collision at %v. Iteration #%v\n", particles[0].position, i)
			}
		}
